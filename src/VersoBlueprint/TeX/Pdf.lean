...
private def removeGeneratedFileIfExists (logError : String → IO Unit)
    (path : System.FilePath) : IO Unit := do
  if ← path.pathExists then
    if ← path.isDir then
      logError s!"Refusing to remove directory at generated PDF output path {path}"
    else
      IO.FS.removeFile path

private def clearGeneratedFiles (logError : String → IO Unit) (pdfDir : System.FilePath) : IO Unit := do
  for filename in generatedFilenames do
    removeGeneratedFileIfExists logError (pdfDir / filename)
...
private def compilePass
  ...
  try
    ...
    if out.exitCode == 0 then
      ...
      logError s!"PDF build failed on pass {pass} with exit code {out.exitCode}.{suffix}"
      pure false
  catch err =>
    logError s!"PDF build failed to start `{opts.engine}`: {err}"
    pure false

def compile (logError : String → IO Unit) (opts : PdfOptions)
    (cfg : Verso.Genre.Manual.Config) : IO Unit := do
  ...
  unless ← texPath.pathExists do
    logError s!"PDF build requested, but TeX output is missing at {texPath}"
    return
  let pdfDir := destination / "pdf"
  IO.FS.createDirAll pdfDir
  clearGeneratedFiles logError pdfDir
  let cacheRoot := destination / "tex-cache"
  for pass in [1:opts.runs + 1] do
    unless ← compilePass logError opts texDir pdfDir cacheRoot pass cfg.verbose do
      return
  ...
  if ← pdfPath.pathExists then
    ...
    logError s!"PDF build completed, but expected output is missing at {pdfPath}"
