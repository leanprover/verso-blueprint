...
The current derived JSON shape is flat, but callers should treat `metadata` and
`renderedBody` as separate concerns. Traversal data is regenerated by the
current Blueprint revision, so this internal representation may change.
-/
