...
private def dumpManifest
  ...
  let (_text, traverseState) ←
    ReaderT.run (Verso.Genre.Manual.traverseHtmlMulti logError traverseCfg text) extensionImpls
  ...
  go : ReaderT ExtensionImpls IO UInt32 := do
    ...
    if cfg.emitTeX then
      withTimedBuildProgress cfg.verbose "TeX emission" <| do
        ...
        Informal.TeX.Cleanup.patchFile logError cfg.toConfig text
    ...
    if pdfOptions.enabled then
      Informal.TeX.Pdf.compile logError pdfOptions cfg.toConfig
